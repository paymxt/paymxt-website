        ...
        <template v-if="blok.logo_in_middle">
            ...
            <div class="col-4 col-md-2 text-right text-md-center order-lg-6">
              <nuxt-link :to="`/${blok.logo_link.cached_url}`"
                          @click.native="$mixpanel.track('logo')"
                          v-if="blok.logo">
                <img :src="blok.logo"
...
<template v-if="!blok.logo_in_middle">
  <div class="col-3 text-left">
    <nuxt-link :to="`/${blok.logo_link.cached_url}`" v-if="blok.logo">
      <img :src="blok.logo" :height="blok.logo_height" alt="Logo">
    </nuxt-link>
  </div>

  <button class="navbar-toggler" type="button" data-toggle="collapse" :data-target="`#navbar-${blok._uid}`" :aria-controls="`navbar-${blok._uid}`" aria-expanded="false" aria-label="Toggle navigation">
              <span class="navbar-toggler-icon"></span>
            </button>

  <div class="collapse navbar-collapse justify-content-center col-md-6" :id="`navbar-${blok._uid}`">
  ...
  <div class="collapse navbar-collapse" :id="`navbar-${blok._uid}`" v-if="blok.button_text">
    <ul v-if="blok.second_nav_links && blok.second_nav_links.length" class="navbar-nav ml-auto justify-content-end">
  ...
  <ul v-if="!blok.button_text && blok.second_nav_links" class="navbar-nav col-3 justify-content-end d-none d-md-flex">
    <component :key="blok._uid" v-for="blok in blok.second_nav_links" :blok="blok" :is="blok.component | dashify"></component>
  </ul>
</template>
      </nav>
